...
def save():
    ...
    try:
        ...
        message = f"Entry saved successfully!"
        if saved_count > 0:
            message += f" {saved_count} photo(s) uploaded."
        
        success_html = f"""
        <div style="text-align:center;margin-top:50px;">
            <h3 style="color:#48bb78;">✅ {message}</h3>
...
def view(id):
    ...
    if entry["user_id"] != session["user_id"] and not session.get("is_admin"):
        return redirect("/")
